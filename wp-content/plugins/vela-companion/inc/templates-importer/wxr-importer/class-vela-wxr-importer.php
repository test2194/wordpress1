 ...
 * Class Vela WXR Importer
 */
class Vela_WXR_Importer {

	/**
	 * Instance of Vela_WXR_Importer
	 *
	 */
	private static $_instance = null;

	/**
	 * Instantiate Vela_WXR_Importer
	 */
	...
	private function __construct() {
		...
		if ( ! class_exists( 'WP_Importer_Logger' ) && !(isset($_GET['post_type'])) ) {
			require_once VELA_COMPANION_DIR . 'inc/templates-importer/wxr-importer/class-logger.php';
		}

		if ( ! class_exists( 'WP_Importer_Logger_ServerSentEvents' ) ) {
			require_once VELA_COMPANION_DIR . 'inc/templates-importer/wxr-importer/class-wp-importer-logger-serversentevents.php';
		}

		if ( ! class_exists( 'WXR_Importer' ) ) {
			require_once VELA_COMPANION_DIR . 'inc/templates-importer/wxr-importer/class-wxr-importer.php';
		}

		if ( ! class_exists( 'WXR_Import_Info' ) ) {
			require_once VELA_COMPANION_DIR . 'inc/templates-importer/wxr-importer/class-wxr-import-info.php';
		}

		add_filter( 'upload_mimes', array( $this, 'custom_upload_mimes' ) );
		add_action( 'wp_ajax_vela-wxr-import', array( $this, 'sse_import' ) );
	...
	public function get_xml_data( $path ) {
		...
		return array(
			...
			'url'     => $url,
			'strings' => array(
				'complete' => __( 'Import complete!', 'vela-companion' ),
			),
		);
	}
	...
	public function get_importer() {
		$options  = apply_filters(
			'vela_sites_xml_import_options', array(
