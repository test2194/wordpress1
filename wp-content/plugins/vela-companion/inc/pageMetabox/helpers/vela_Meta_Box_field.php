...
class vela_Meta_Box_field {
	...
	public function __construct( $field_args, $group_field = null ) {
		$this->object_id   = vela_Meta_Box::get_object_id();
		$this->object_type = vela_Meta_Box::get_object_type();
	...
	public function get_data( $field_id = '', $args = array() ) {
		...
		$data = 'options-page' === $type
			? vela_Meta_Box::get_option( $id, $field_id )
	...
	public function update_data( $new_value, $single = true ) {
		...
		if ( 'options-page' === $type )
			return vela_Meta_Box::update_option( $id, $field_id, $new_value, $single );
	...
	public function remove_data( $old = '' ) {
		...
		return 'options-page' === $type
			? vela_Meta_Box::remove_option( $id, $field_id )
	...
	public function sanitization_cb( $meta_value ) {
		...
		$clean = new vela_Meta_Box_Sanitize( $this, $meta_value );
		// Validation via 'vela_Meta_Box_Sanitize' (with fallback filter)
	...
	public function field_timezone_offset() {
		return vela_Meta_Box::timezone_offset( $this->field_timezone() );
	}
	...
	public function render_field() {
		...
		$this_type = new vela_Meta_Box_types( $this );
		$this_type->render();
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => __( 'Add Group', 'vela-companion' ),
			'remove_button' => __( 'Remove Group', 'vela-companion' ),
		) ) : $args['options'];
