 ...
 * @since  0.0.4
 */
class vela_Meta_Box_Sanitize {

	/**
	 * A CMB field object
	 * @var vela_Meta_Box_field object
	 */
	public $field;
	...
	public function text_datetime_timestamp_timezone( $value, $repeat = false ) {
		...
		if ( empty( $tzstring ) )
			$tzstring = vela_Meta_Box::timezone_string();

		$offset = vela_Meta_Box::timezone_offset( $tzstring, true );
	...
	public function _save_file_id( $value ) {
		...
		$field      = new vela_Meta_Box_field( $args, $group );
		$id_key     = $field->_id();
		...
		if ( $value && ! $id_val ) {
			$id_val = vela_Meta_Box::image_id_from_url( $value );
		}
