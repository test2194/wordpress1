...
class vela_Meta_Box_ajax {
	 ...
	 * @since  0.1.0
	 * @return vela_Meta_Box_ajax A single instance of this class.
	 */
	...
	public function oembed_handler() {
		...
		if ( empty( $oembed_string ) )
			self::send_result( '<p class="ui-state-error-text">'. __( 'Please Try Again', 'vela-companion' ) .'</p>', false );
	...
	public static function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'vela_Meta_Box_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'vela_Meta_Box_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="'. $args['field_id'] .'">'. __( 'Remove Embed', 'vela-companion' ) .'</a></p></div>';

		// Otherwise, send back error info that no oEmbeds were found
		return '<p class="ui-state-error-text">'. sprintf( __( 'No oEmbed Results Found for %s. View more info at', 'vela-companion' ), $fallback ) .' <a href="http://codex.wordpress.org/Embeds" target="_blank">codex.wordpress.org/Embeds</a>.</p>';
	...
	public static function hijack_oembed_cache_get( $check, $object_id, $meta_key ) {
		...
		$data = 'options-page' === self::$object_type
			? vela_Meta_Box::get_option( self::$object_id, self::$embed_args['cache_key'] )
	...
	public static function hijack_oembed_cache_set( $check, $object_id, $meta_key, $meta_value ) {
		...
		if ( 'options-page' === self::$object_type ) {
			// Set the option
			vela_Meta_Box::update_option( self::$object_id, self::$embed_args['cache_key'], $meta_value, array( 'type' => 'oembed' ) );
			// Save the option
			vela_Meta_Box::save_option( self::$object_id );
		} else {
