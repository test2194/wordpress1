...
class vela_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php _e( 'Add Row', 'vela-companion' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php _e( 'Remove', 'vela-companion' ); ?></a>
			</td>
		</tr>
	...
	public function colorpicker() {
		...
		return $this->input( array( 'class' => 'cmb_colorpicker cmb_text_small color-picker', 'data-alpha' => 'true', 'value' => $meta_value ) );
	}
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms', 'vela-companion' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => __( 'Add or Upload File', 'vela-companion' ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image', 'vela-companion' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						__( 'File:', 'vela-companion' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'vela-companion' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. __( 'Remove', 'vela-companion' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File', 'vela-companion' ) .'" />',
		$this->_desc( true );
		...
		$this->field = new vela_Meta_Box_field( $args, $this->field->group );
		...
		if ( $meta_value && ! $_id_value ) {
			$_id_value = vela_Meta_Box::image_id_from_url( esc_url_raw( $meta_value ) );
		}
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove Image', 'vela-companion' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'vela-companion' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. __( 'Download', 'vela-companion' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove', 'vela-companion' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public function oembed() {
		...
		'<div id="',$this->_id( '_status' ) ,'" class="cmb_media_status ui-helper-clearfix embed_wrap">';
			...
			if ( $meta_value = $this->field->escaped_value() ) {
				echo vela_Meta_Box_ajax::get_oembed( $meta_value, $this->field->object_id, array(
