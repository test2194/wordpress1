...
spl_autoload_register('vela_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'cmb_meta_boxes', $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new vela_Meta_Box( $meta_box );
}

define( 'VELA_META_BOX_URL', vela_Meta_Box::get_meta_box_url() );
...
class vela_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		$show_filters = 'vela_Meta_Box_Show_Filters';
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'vela-companion'  ),
					'defaultString' => __( 'Default', 'vela-companion'  ),
					'pick'          => __( 'Select Color', 'vela-companion'  ),
					'current'       => __( 'Current Color', 'vela-companion'  ),
				) );
			}
		} else {
		...
		wp_register_script( 'cmb-datepicker', VELA_META_BOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'cmb-timepicker', VELA_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'cmb-scripts', VELA_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, '' );
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __( 'Use this file', 'vela-companion' ),
			'remove_image'    => __( 'Remove Image', 'vela-companion' ),
			'remove_file'     => __( 'Remove', 'vela-companion' ),
			'file'            => __( 'File:', 'vela-companion' ),
			'download'        => __( 'Download', 'vela-companion' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All', 'vela-companion' ),
		) ) );
		
		wp_enqueue_script( 'wp-color-picker-alpha', VELA_META_BOX_URL . 'js/wp-color-picker-alpha.js', array( 'wp-color-picker' ), '', '' );

		wp_register_style( 'cmb-styles', VELA_META_BOX_URL . 'style'. $min .'.css', $styles );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		foreach ( $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new vela_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new vela_Meta_Box_field( $args );
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new vela_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
...
// Handle oembed Ajax
add_action( 'wp_ajax_cmb_oembed_handler', array( 'vela_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cmb_oembed_handler', array( 'vela_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function vela_get_option( $option_key, $field_id = '' ) {
	return vela_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              vela_Meta_Box_field object
 */
function vela_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	vela_Meta_Box::set_object_id( $object_id );
	vela_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new vela_Meta_Box_field( $field_args );
}
 ...
 */
function vela_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
	$field = vela_get_field( $field_args, $object_id, $object_type );
 ...
 * @param int   $object_id  Object ID
 */
function vela_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		vela_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function vela_print_metabox( $meta_box, $object_id ) {
	$cmb = new vela_Meta_Box( $meta_box );
	if ( $cmb ) {
		...
		vela_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function vela_save_metabox_fields( $meta_box, $object_id ) {
	vela_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function vela_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = vela_Meta_Box::set_mb_defaults( $meta_box );
	...
	vela_Meta_Box::set_object_type( vela_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], vela_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		vela_save_metabox_fields( $meta_box, $object_id );
