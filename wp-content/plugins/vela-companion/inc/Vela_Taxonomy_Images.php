<?php
if( ! class_exists( 'Vela_Taxonomy_Images' ) ) {
  class Vela_Taxonomy_Images {
   ...
   public function save_category_image( $term_id, $tt_id ) {
     if( isset( $_POST['vela-taxonomy-image-id'] ) && '' !== $_POST['vela-taxonomy-image-id'] ){
       add_term_meta( $term_id, 'vela-taxonomy-image-id', absint( $_POST['vela-taxonomy-image-id'] ), true );
     }
    }
    ...
    public function update_category_image( $term, $taxonomy ='' ) { ?>
      <tr class="form-field term-group-wrap">
        <th scope="row">
          <label for="vela-taxonomy-image-id"><?php _e( 'Background Image', 'vela-companion' ); ?></label>
        </th>
        <td>
          <?php $image_id = get_term_meta( $term->term_id, 'vela-taxonomy-image-id', true ); ?>
          <input type="hidden" id="vela-taxonomy-image-id" name="vela_category_meta[<?php echo $term->term_id ?>][bg_img]" value="<?php echo esc_attr( $image_id ); ?>">
   ...
   public function updated_category_image( $term_id, $tt_id ) {
     if( isset( $_POST['vela-taxonomy-image-id'] ) && '' !== $_POST['vela-taxonomy-image-id'] ){
       update_term_meta( $term_id, 'vela-taxonomy-image-id', absint( $_POST['vela-taxonomy-image-id'] ) );
     } else {
       update_term_meta( $term_id, 'vela-taxonomy-image-id', '' );
     }
   }
       ...
       function ct_media_upload(button_class) {
         ...
         $('body').on('click', button_class, function(e) {
           ...
           wp.media.editor.send.attachment = function(props, attachment){
             if( _custom_media ) {
               $('#vela-taxonomy-image-id').val(attachment.id);
...
$Vela_Taxonomy_Images = new Vela_Taxonomy_Images();
$Vela_Taxonomy_Images->init(); 
}