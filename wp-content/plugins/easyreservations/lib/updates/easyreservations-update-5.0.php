...
foreach( $reservations as $reservation ) {
    delete_reservation_meta($reservation->id, 'custom');
    delete_reservation_meta($reservation->id, 'chat');
    delete_reservation_meta($reservation->id, 'history');
    delete_reservation_meta($reservation->id, 'coupon');
    delete_reservation_meta($reservation->id, 'slot');
    delete_reservation_meta($reservation->id, 'credit');
    delete_reservation_meta($reservation->id, 'invoice');
			...
			if( $custom["type"] == 'cstm' ) {
				if( ! isset( $custom['id'] ) ) {
					if( ! isset( $new_custom_fields[ $custom['title'] ] ) ) {
						...
						$custom_fields['fields'][ $custom_fields['id'] ] = array( 'title'  => $custom['title'], 'type'   => 'text',  'value'  => '', 'unused' => '');
