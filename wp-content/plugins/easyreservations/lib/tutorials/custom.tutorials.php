<?php
	function easyreservations_custom_tutorial() {
		$handler = array('#wpbody a.active','#custom_price_field', '#custom_field_type', 'input[name="value[]"]', 'input[name="price[]"]', '#add_new_custom', '#add_if_clause', 'select[name="if_cond_type[]"]', 'select[name="if_cond_happens[]"]', 'select[name="if_cond_mult[]"]', 'input[name=custom_field_unused]');
		$content = array(
				'<h3>Custom fields</h3><p>With custom fields you can add your own fields to the form easily. The data gathered can be used through the whole system like in the emails, invoices and when the guest edits his reservation.</p>',
