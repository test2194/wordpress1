<?php
	function easyreservations_paypal_tutorial() {
		$handler = array('#wpbody a.active','select[name="er_pay_modus"]', 'select[name="language"]', 'input[name="deposit_payown"]');
		$content = array(
				'<h3>PayPal</h3><p>After activating and setting an owner the <b>Buy now!</b> button will appear after successful reservations and when the guest edits his reservation.</p>',
				...
				'<h3>Language</h3><p>Select the language in that the PayPal site should be before your guests login.</p>',
