...
function insertNewLine(line){

	window.setTimeout( insertNewLine, 5000 );
}
...
jQuery('#form_container')
	...
	/* 90% working paste - but it seems normals paste works flawlessly right now
	...
	.bind("paste",function(e){
		...
		if( e.originalEvent.clipboardData ){
			...
			for(var i in items){
				if(i !== items.length){
					document.execCommand('insertText', false, items[i].trim());
					document.execCommand('insertHTML', false, '<br>');
					var hasBR = contentHasBr(document.getElementById('form_container'));
					if(!hasBR){
						console.log('another');
...
function contentHasBr(containerEl) {
	var sel, range;
	if (window.getSelection) {
		...
		if (sel.rangeCount > 0) {
			...
			var child_nodes = range.cloneContents().childNodes;
			console.log(child_nodes);
			if(child_nodes[0].nodeName == 'BR'){
				return true;
			}
			return false;
		}
...
function generateTagEdit(type, tag){
	if(fields[type]){
			...
			jQuery.each(options, function(k,v){
				...
				if(typeof v['input'] == 'function') value += v['input'](tag);
					...
					if(v['input'] == 'text') {
						...
						value += '<label class="wrapper"><input type="checkbox" name="'+k+'" value="'+v['default']+'" '+sel+hasclass+'><span class="input"></span>'+v['title']+'</label>';
					} else if(v['input'] == 'select'){
						value += '<span class="select"><select name="'+k+'"'+hasclass+'>';
		...
		value += '<div class="easy-ui" style="line-height: 34px;margin-top: 5px">';
		value += '<a href="javascript:" class="easy-button" onclick="submitTag();">'+title+'</a>&nbsp;';
		value += '<a href="javascript:" class="easy-button grey" onclick="deactivateTag();">Cancel</a>';
		value += '</div>';
		value += '</div>';
		jQuery('#accordion').prepend(value).accordion( "destroy").accordion({heightStyle: "content", autoHeight: false, icons: { "header": "ui-icon-plus", "activeHeader": "ui-icon-minus" }});
	}
}
