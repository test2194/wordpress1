...
function custom_field_extras(sel){
	...
	html += '<p><label class="wrapper"><input type="checkbox" id="custom_field_required" name="custom_field_required"'+selected+'><span class="input"></span> Required</label></p>';
	selected = '';
	if(sel && sel['admin']) selected = ' checked="checked"';
	html += '<p><label class="wrapper"><input type="checkbox" id="custom_field_admin" name="custom_field_admin"'+selected+'><span class="input"></span> Display when adding reservation as admin</label></p>';
	//if(jQuery('#custom_price_field').prop('checked') )
...
function custom_field_value(sel){
	...
	if(type !== 'x'){
		...
		if(type == 'text' || type == 'area'){
			...
			if(type == 'select' || type == 'radio' || type == 'check') options += '<strong>Options</strong> <a id="add_new_custom" onclick="add_new_option()" style="font-size:14px;" class="fa fa-plus-circle"></a>';
...
function custom_generate_option(sel){
	...
	for(var k in sel['options']){
		...
		if(sel['type'] == 'check' || sel['type'] == 'select' || sel['type'] == 'radio') options += '<a style="float:right" onclick="delete_option(this)" class="fa fa-times-circle"></a>';
		...
		if(jQuery('#custom_price_field').is(':checked') && (sel['type'] == 'number' || sel['type'] == 'slider')){
			if(v['mode']) checked = ' checked="checked"';
			options += '<br><label class="wrapper" style="margin-left:14px"><input type="checkbox" name="number-price[]" '+checked+' value="1"><span class="input"></span>Multiply this options price by fields value </label>';
		}	else if(sel['type'] == 'check' || sel['type'] == 'select' || sel['type'] == 'radio'){
			if(v['checked']) checked = ' checked="checked"';
			options += '<br><label class="in-hierarchy">Checked</label><label class="wrapper"><input type="checkbox" name="checked[]"'+checked+' onchange="check_checkboxes(this)" value="1"><span class="input"></span></label>';
		}
		else options += '<input type="hidden" name="checked[]" value="0">';
		if(jQuery('#custom_price_field').is(':checked')){
			options += '<br><label class="in-hierarchy">Price</label><span class="input-wrapper"><input type="text" name="price[]" value="'+v["price"]+'" style="width:50px;text-align:right"><span class="input-box">&'+currency+';</span></span>';
			options += '<br><strong>Conditions' +	'</strong> <a id="add_if_clause" onclick="add_if_clause(\''+k+'\');" style="font-size: 14px;" class="fa fa-plus-circle"></a><ul id="clauses_sortable'+k+'">';
			if(v['clauses']){
...
function add_if_clause(id, sel){
	...
	clause += '<a style="float:right" onclick="delete_option(this)" class="fa fa-times-circle"></a>';
	...
	clause += '<span class="select"><select name="if_cond_happens[]" onchange="clause_happens_select(jQuery(this).val(),\''+id+'\', '+ifTempID+', true)">'+generateOptions({x:"--",and:"AND",or:"OR",price:"THEN"}, selection)+'</select></span>';
...
function generate_if_clause_condition(sel){
	...
	if(sel['type'] == 'field'){
		...
		if(!select) clause += '<b name="if_cond[]">Add price fields first</b>';
			...
			clause += ' <span class="select delete"><select name="if_cond_operator[]" onchange="change_condition_options(this)" class="easy-tooltip"  title="Other price field that has to be selected">'+ select[0] + '</select></span><span class="delete"> and option </span>';
			clause += ' <span class="select delete"><select name="if_cond[]" class="easy-tooltip" title="Price fields option that has to be selected">' + generate_customs_options_select(select[1], sel['cond']) + '</select></span><span class="delete"> are selected </span>';
		}
	} else if(sel['type'] == 'resource'){
		clause += '<span class="select delete"><select name="if_cond_operator[]">'+generateOptions({equal:"=", notequal:"!="}, sel['operator'])+'</select></span> ';
		clause += '<span class="select delete"><select name="if_cond[]" class="easy-tooltip" title="Resource that has to be selected">'+generateOptions(resources, sel['cond'])+'</select></span>';
	} else {
		clause += '<span class="select delete"><select name="if_cond_operator[]">'+generateOptions({equal:"=", notequal:"!=", greater:">", greaterequal:">=",smaller:"<", smallerequal:"<="}, sel['operator'])+'</select></span> ';
		if(sel['type'] == 'arrival' || sel['type'] == 'departure'){
			clause += '<input type="text" name="if_cond[]" onclick="generate_datepicker(this);" value="'+sel['cond']+'" style="width:100px;text-align:center"> ';
		} else if(sel['type'] == 'arrival_every' || sel['type'] == 'departure_every') {
			clause += '<input type="text" name="if_cond[]" onclick="generate_datepicker_without_year(this);" value="'+sel['cond']+'" style="width:100px;text-align:center"> ';
		} else {
			clause += '<input type="text" name="if_cond[]" class="easy-tooltip" title="Number that has to be matched" value="'+sel['cond']+'" style="width:50px;text-align:center"> ';
		}
	}
	return clause;
}
...
function clause_happens_select(sel, opt_id, clause_id, append, mult){
	...
	if(sel == 'price' || jQuery.isNumeric(sel)){
		...
		content += '<span class="input-wrapper delete"><input type="text" name="if_cond_amount[]" id="if_clause_amount_'+opt_id+'_'+clause_id+'" value="'+sel+'" style="width:55px;text-align:right"><span class="input-box">&'+currency+';</span></span> per ';
...
function generate_datepicker_without_year(e){
	jQuery(e).datepicker({dateFormat:'mm.dd'}).datepicker("show");
}
