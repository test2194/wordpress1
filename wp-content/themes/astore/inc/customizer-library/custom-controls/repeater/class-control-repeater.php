...
add_action( 'customize_register', 'astore_repeater_customize_register' );
function astore_repeater_customize_register(){
/**
 * Repeater control
 */
class AStore_Control_Repeater extends WP_Customize_Control {
	...
	/**
	 * The astore_config we're using for this control
	 *
	 * @access public
	 * @var string
	 */
	public $astore_config = 'global';
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		if ( empty( $this->button_label ) ) {
			...
			$this->button_label = sprintf( esc_attr__( 'Add new %s', 'astore' ), $this->row_label['value'] );
		}
					...
					case 'dropdown-pages':
						...
						$dropdown = wp_dropdown_pages(
							array(
								'name'              => '',
								'echo'              => 0,
								'show_option_none'  => esc_attr__( 'Select a Page', 'astore' ),
								'option_none_value' => '0',
								'selected'          => '',
							)
						);
	...
	public function enqueue() {
				...
				if ( isset( $field['type'] ) ) {
					...
					switch ( $field['type'] ) {
						case 'color':
							...
							//case 'select':
						case 'dropdown-pages':
							wp_enqueue_script( 'select2', CUSTOMIZER_LIB_URI . '/assets/vendor/select2/js/select2.full.js', array( 'jquery' ), '4.0.3', true );
							wp_enqueue_style( 'select2', CUSTOMIZER_LIB_URI . '/assets/vendor/select2/css/select2.css', array(), '4.0.3' );
							wp_enqueue_style( 'astore-select2', CUSTOMIZER_LIB_URI . '/assets/vendor/select2/astore.css', null );
							break;
						case 'iconpicker':
		...
		wp_enqueue_script( 'astore-repeater', CUSTOMIZER_LIB_URI. '/custom-controls/repeater/repeater.js', array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable' ), false, true );
		wp_enqueue_style( 'astore-repeater-css', CUSTOMIZER_LIB_URI . '/custom-controls/repeater/repeater.css', null );
	}
	...
	protected function render_content() {
		?>
		<div class="astore-controls-loading-spinner"><div class="bounce1"></div><div class="bounce2"></div><div class="bounce3"></div></div>
		<label>
		...
		<?php if ( isset( $this->choices['limit'] ) ) : ?>
			...
			<p class="limit"><?php printf( esc_attr__( 'Limit: %s rows', 'astore' ), esc_html( $this->choices['limit'] ) ); ?></p>
		<?php endif; ?>
		...
		<script type="text/html" class="customize-control-repeater-content">
			...
			<li class="repeater-row minimized" data-row="{{{ index }}}">
				...
				<div class="repeater-row-header">
					...
					<span class="repeater-row-remove" style="cursor:pointer;"><?php esc_attr_e( 'Remove', 'astore' ); ?></span>
				...
				<div class="repeater-row-content">
						...
						<div class="repeater-field repeater-field-{{{ field.type }}}">
							...
							<# } else if ( 'color' === field.type ) { #>
								...
								<label>
									<# if ( field.label ) { #><span class="customize-control-title">{{{ field.label }}}</span><# } #>
									<# if ( field.description ) { #><span class="description customize-control-description">{{{ field.description }}}</span><# } #>
									<input class="color-picker-hex" type="text" maxlength="7" placeholder="<?php esc_attr_e( 'Hex Value', 'astore' ); ?>"  value="{{{ field.default }}}" data-field="{{{ field.id }}}" {{ defaultValue }} />
							...
							<# } else if ( 'textarea' === field.type ) { #>
								...
								<# } else if ( 'editor' === field.type ) { #>
									 <div class="customize-control-astore-editor">
							...
							<# } else if ( field.type === 'image' || field.type === 'cropped_image' ) { #>
								...
								<figure class="astore-image-attachment" data-placeholder="<?php esc_attr_e( 'No Image Selected', 'astore' ); ?>" >
									<# if ( field.default ) { #>
										<# var defaultImageURL = ( field.default.url ) ? field.default.url : field.default; #>
										<img src="{{{ defaultImageURL }}}">
									<# } else { #>
										<?php esc_attr_e( 'No Image Selected', 'astore' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"><?php esc_attr_e( 'Remove', 'astore' ); ?></button>
									<button type="button" class="button upload-button" data-label=" <?php esc_attr_e( 'Add Image', 'astore' ); ?>" data-alt-label="<?php echo esc_attr_e( 'Change Image', 'astore' ); ?>" >
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change Image', 'astore' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add Image', 'astore' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
							...
							<# } else if ( field.type === 'upload' ) { #>
								...
								<figure class="astore-file-attachment" data-placeholder="<?php esc_attr_e( 'No File Selected', 'astore' ); ?>" >
									<# if ( field.default ) { #>
									...
									<# } else { #>
										<?php esc_attr_e( 'No File Selected', 'astore' ); ?>
									<# } #>
								</figure>

								<div class="actions">
									<button type="button" class="button remove-button<# if ( ! field.default ) { #> hidden<# } #>"></button>
									<button type="button" class="button upload-button" data-label="<?php esc_attr_e( 'Add File', 'astore' ); ?>" data-alt-label="<?php esc_attr_e( 'Change File', 'astore' ); ?>">
										<# if ( field.default ) { #>
											<?php esc_attr_e( 'Change File', 'astore' ); ?>
										<# } else { #>
											<?php esc_attr_e( 'Add File', 'astore' ); ?>
										<# } #>
									</button>
									<# if ( field.default.id ) { #>
										<input type="hidden" class="hidden-field" value="{{{ field.default.id }}}" data-field="{{{ field.id }}}" >
							...
							<# } else if ( 'iconpicker' === field.type ) { #>
									...
									<div class="input-group icp-container">
										<input data-placement="bottomRight" class="icp icp-auto" value="{{{ field.default }}}" type="text" data-field="{{{ field.id }}}"{{ fieldExtras }}>
					...
					<# }); #>
					<button type="button" class="button-link repeater-row-remove"><?php esc_attr_e( 'Remove', 'astore' ); ?></button>
				</div>
			</li>
		</script>
		<?php
