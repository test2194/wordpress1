...
wp.customize.controlConstructor.repeater = wp.customize.Control.extend({
	...
	ready: function() {
		...
		// Init the control.
		if ( ! _.isUndefined( window.astoreControlLoader ) && _.isFunction( astoreControlLoader ) ) {
			astoreControlLoader( control );
		} else {
			control.initAStoreControl();
		}
	},

	initAStoreControl: function() {
		...
		// The current value set in Control Class (set in AStore_Customize_Repeater_Control::to_json() function)
		var settingValue = this.params.value;

		control.container.find( '.astore-controls-loading-spinner' ).hide();
	...
	setImageInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.astore-image-attachment' ).html( '<img src="' + attachment.url + '">' ).hide().slideDown( 'slow' );
	...
	setFileInRepeaterField: function( attachment ) {
		...
		$targetDiv.find( '.astore-file-attachment' ).html( '<span class="file"><span class="dashicons dashicons-media-default"></span> ' + attachment.filename + '</span>' ).hide().slideDown( 'slow' );
	...
	removeImage: function( event ) {
		...
		$targetDiv.find( '.astore-image-attachment' ).slideUp( 'fast', function() {
	...
	removeFile: function( event ) {
		...
		$targetDiv.find( '.astore-file-attachment' ).slideUp( 'fast', function() {
			jQuery( this ).show().html( jQuery( this ).data( 'placeholder' ) );
		});
		$targetDiv.find( '.hidden-field' ).val( '' );
		$uploadButton.text( $uploadButton.data( 'label' ) );
	...
	addRow: function( data ) {
		...
		var control       = this,
		    template      = control.repeaterTemplate(), // The template for the new row (defined on AStore_Customize_Repeater_Control::render_content() ).
