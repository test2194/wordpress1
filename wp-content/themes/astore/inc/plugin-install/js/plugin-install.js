...
jQuery(document).on('DOMNodeInserted','.activate-now', function (e) {
    ...
    if (activateButton.length) {
        ...
        if (typeof url !== 'undefined') {
            ...
            jQuery.ajax({
                beforeSend: function () {
                    jQuery(activateButton).replaceWith('<a class="button updating-message">' + astore_plugin_helper.activating + '...</a>');
                },
                async: true,
                type: 'GET',
                url: url,
    ...
    $('.activate-now').on('click', function (e) {
        ...
        if ($(activateButton).length) {
            ...
            if (typeof url !== 'undefined') {
                ...
                $.ajax({
                    beforeSend: function () {
                        $(activateButton).replaceWith('<a class="button updating-message">' + astore_plugin_helper.activating + '...</a>');
                    },
                    async: true,
                    type: 'GET',
                    url: url,
