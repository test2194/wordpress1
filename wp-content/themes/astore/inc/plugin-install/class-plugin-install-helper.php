...
class AStore_Plugin_Install_Helper {
	 ...
	 *
	 * @return AStore_Plugin_Install_Helper
	 */
	public static function instance() {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof AStore_Plugin_Install_Helper ) ) {
			self::$instance = new AStore_Plugin_Install_Helper;
		}
		...
		if ( ! empty( $slug ) ) {
			...
			switch ( $state ) {
				...
				case 'activate':
					...
					$button .= '<a data-slug="' . $slug . '" class="activate-now button button-primary" href="' . esc_url( $nonce ) . '" aria-label="Activate ' . $slug . '">' . __( 'Activate', 'astore' ) . '</a>';
					break;
			}// End switch().
			$button .= '</div>';
		}// End if().
	...
	public static function enqueue_scripts() {
		...
		wp_enqueue_script( 'astore-plugin-install-helper', get_template_directory_uri() . '/inc/plugin-install/js/plugin-install.js', array( 'jquery' ), '', true );
		wp_localize_script(
			'astore-plugin-install-helper', 'astore_plugin_helper',
			array(
				'activating' => esc_html__( 'Activating ', 'astore' ),
			)
		);
		wp_localize_script(
			'astore-plugin-install-helper', 'pagenow',
			array( 'import' )
		);
	}
}