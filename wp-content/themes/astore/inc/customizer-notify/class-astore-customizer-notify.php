 ...
 */
class AStore_Customizer_Notify {
	...
	/**
	 * The single instance of AStore_Customizer_Notify
	 *
	 * @var AStore_Customizer_Notify $instance The AStore_Customizer_Notify instance.
	 */
	...
	/**
	 * The Main AStore_Customizer_Notify instance.
	 *
	 * We make sure that only one instance of AStore_Customizer_Notify exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof AStore_Customizer_Notify ) ) {
			self::$instance = new AStore_Customizer_Notify;
	...
	public function setup_actions() {
		...
		add_action( 'customize_register', array( $this, 'astore_customizer_notify_customize_register' ) );

		// Enqueue scripts and styles
		add_action( 'customize_controls_enqueue_scripts', array( $this, 'astore_customizer_notify_scripts_for_customizer' ), 0 );

		/* ajax callback for dismissable recommended actions */
		add_action( 'wp_ajax_cc_customizer_notify_dismiss_recommended_action', array( $this, 'astore_customizer_notify_dismiss_recommended_action_callback' ) );

		add_action( 'wp_ajax_cc_customizer_notify_dismiss_recommended_plugins', array( $this, 'astore_customizer_notify_dismiss_recommended_plugins_callback' ) );

	}

	/**
	 * Scripts and styles used in the AStore_Customizer_Notify class
	 */
	public function astore_customizer_notify_scripts_for_customizer() {
		...
		wp_localize_script(
			'customizer-notify-customizer-js', 'ccCustomizerNotifyObject', array(
				...
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'astore' ),
			)
		);
	 ...
	 */
	public function astore_customizer_notify_customize_register( $wp_customize ) {

		/**
		 * Include the AStore_Customizer_Notify_Section class.
		 */
		require_once get_template_directory() . '/inc/customizer-notify/class-astore-customizer-notify-section.php';

		$wp_customize->register_section_type( 'AStore_Customizer_Notify_Section' );

		$wp_customize->add_section(
			new AStore_Customizer_Notify_Section(
				$wp_customize,
	 ...
	 */
	public function astore_customizer_notify_dismiss_recommended_action_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			if ( get_option( 'astore_customizer_notify_show_recommended_actions' ) ) {
				...
				update_option( 'astore_customizer_notify_show_recommended_actions', $cc_customizer_notify_show_recommended_actions );
				...
				if ( ! empty( $cc_customizer_notify_recommended_actions ) ) {
					...
					update_option( 'astore_customizer_notify_show_recommended_actions', $cc_customizer_notify_show_recommended_actions_new );
				}
			}
		}
	 ...
	 */
	public function astore_customizer_notify_dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'astore_customizer_notify_show_recommended_plugins', $cc_customizer_notify_show_recommended_plugins );
		}
		die(); // this is required to return a proper result
	}
